...
public class Durak {
	...
	void placeCardAttacker(Card card)
	{
		...
		else if(card.getRank().equalsIgnoreCase(currentTable.get(currentCard).getRank()) || Assignment.cardValues.get(card.getRank()) == Assignment.cardValues.get(currentTable.get(currentCard).getRank()))
		{
	...
	void placeCardDefender(Card defCard)
	{
		...
		int player = 0;
		{
			...
			if(attCard.compareTo(defCard) == 1) //compareTo benutzen!
			{

				currentTable.add(attCard);
				//card.move(x,y);
				System.out.println("Defender Spieler");
			...
			else if(attCard.compareTo(defCard) == -1)
			{
				System.out.println("Angreifer "+attCard+" Verteidiger "+defCard);
	...
	void checkLooser(){
		...
		for(int i = 0; i < playerNumber; i++){
			System.out.println("Spieler ohne Karten");
